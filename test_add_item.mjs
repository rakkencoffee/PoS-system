...
async function run() {
  ...
  const url = OLSERA_API_BASE + '/api/open-api/v1/en/order/openorder/item';

  const formData = new URLSearchParams();
  formData.append('order_id', '12557879'); // Assuming from previous logs
  formData.append('product_id', '1'); // Try dummy product
  ...
  console.log('Testing addItem', r.status);
