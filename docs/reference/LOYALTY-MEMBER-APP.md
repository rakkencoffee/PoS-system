...
| 7 | `/api/member/:id/redeem` | POST | ✅ DONE (sebagian — lihat catatan) | Cek saldo poin, tulis `PointLedger` REDEEM, decrement `Member.pointBalance`. **BELUM generate Discount Voucher Olsera beneran** — `olsera.service.ts` gak punya fungsi create-voucher, nebak bentuk request-nya terlalu beresiko (bisa nulis data salah ke Olsera asli), jadi response balikin `voucherIssued:false` dulu. `stockQuota` di `RewardsCatalog` juga BELUM di-enforce (gak ada counter penukaran). |
